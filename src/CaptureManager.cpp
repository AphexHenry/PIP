...
//            lDistanceCalib = (float)(calibIter.r() + calibIter.g() + calibIter.b()) / (float)(inputIter.r() + inputIter.g() + inputIter.b());
            ...
            {
                lPositionsAll.push_back(vec3(current.x / (float)mSurface->getWidth(),(float)current.y / (float)mSurface->getWidth(),Rand::randFloat(-0.5f, 0.5f)));
            }
    ...
    else
    {
        ...
        {
            Shared::sAnchors[i]->SetPosition(vec3(100.5f + Rand::randFloat(1.f), 10.f, 0.f));
        }
    }
}
